...
float Enemy::checkTargetDesireable()
{
	float desire = 0;
...
float Enemy::findNearestTarget(WorldObjectType type)
{
	float minDistance = 1000000;
	for (auto object : *worldObjects)
	{
...
glm::vec2 Enemy::findTargetVector(WorldObjectType type)
{
	float minDistance = 1000000;
	glm::vec2 target;
	for (auto object : *worldObjects)
...
glm::vec2 Enemy::gotoAgentTarget(float desirability, float delta)
{
